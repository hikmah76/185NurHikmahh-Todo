    ...
    <div class="py-12">
        <div class="max-w-7xl mx auto sm:px-6 lg:px-8">
            <div class="bg-white drak:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 drak:text-grey-100">
                    {{ __("Index User Page") }}
                </div>
            </div>
        </div>
            ...
            <tbody>
                @forelse ($users as $user)
                    <tr class="odd:bg-white odd:dark:bg-gray-800 even:bg-gray-50 even:dark:bg-gray-700">
                        ...
                        <td class="px-6 py-4 font-medium text-gray-900 md:whitespace-nowrap drak:text-white">
                            <p>{{ $user->name }}</p>
                        </td>
                        ...
                        <td class="hidden px-6 py-4 whitespace-nowrap">
                            <p>{{ $user->todos->count() }}
                                <span>
                                    <span class="text-green-600 drak:text-green-400">({{
                                    $user->todos->where('is_complete', true)->count()}}</span>/
                                <span
                                    class="text-blue-600 drak:text-blue-400"{{$user->todos->where('is_complete',
                                    false)->count()}})</span>
                                </span>
                    ...
                    @empty 
                    <tr class="bg-white dark:dg-gray-800">
