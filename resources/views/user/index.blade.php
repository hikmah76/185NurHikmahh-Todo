<x-app-layout>
                                        ...
                                        <div class="flex space-x-2">
                                            @if ($user->is_admin)
                                                <form action="{{ route('user.removeadmin', $user) }}" method="POST">
                                                    @csrf
                                                    @method('PATCH') <!-- Metode PATCH untuk Remove Admin -->
                                                    <button type="submit" class="text-yellow-600 dark:text-yellow-400">
                                                        Remove Admin
                                                    </button>
                                                </form>
                                            @else
                                                <form action="{{ route('user.makeadmin', $user) }}" method="POST">
                                                    @csrf
                                                    @method('PATCH') <!-- Metode PATCH untuk Make Admin -->
    ...
    <div class="overflow-hidden bg-white shadow-sm dark:bg-gray-800 sm:rounded-lg">
        <div class="px-6 pt-6 md:5 md:w-1/2 xl:w-1/3">
            @if (request('search'))
            ...
            <form class="flex items-center gap-2">
                <x-text-input id="search" name="search" type="text" class="w-full" placeholder="Cari berdasarkan nama atau email ..." value="{{ request('search') }}" autofocus />
                <x-primary-button type="submit">{{ __('Search') }}</x-primary-button>
            </form>
        </div>

        <div class="px-5 text-xl text-gray-900 dark:text-gray-100">
